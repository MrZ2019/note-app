...
.controller('MainCtrl', function($scope, $rootScope,  $http,
			...
			db.transaction(function(tx) {
				...
				tx.executeSql(sql, [],
					function(tx, result) {
						...
						// var sql = 'insert into ' + table + '(content, category, datepost, status, guid) values(?, ?, datetime("now"),?, ?)'
    ...
    $rootScope.theme=localStorage.getItem('theme') || 'orange';
	...
	$rootScope.saveSetting = function() {
		...
		window.config.dataUrl = 'http://' + setting.host + ':7002/api/getdata'
		window.config.appDataUrl = 'http://' + setting.host + ':7002/api/getappdata'
	}
	...
	$rootScope.remove = function($event, row, data, flag) {
		if((flag == 'slide') || $event.gesture.direction == 'right')
		{
